...
<script type="text/javascript">
    ...
    setInterval(function(){
        var sendersid = window.location.hash;
        $.post("<?= site_url('admin/member_chat_admin/getRecepients'); ?>",{},function(e){
            // alert('hello');
            $(".chat-user").html(e);
        });
        if(sendersid != ""){
            sendersid = sendersid.replace("#","");
            $.post("<?= site_url('admin/member_chat_admin/member_message_detail'); ?>", {id: sendersid}, function (e) {
                $("#msgs").html(e);
    ...
    function sendMsg(){
        ...
        var msg = $("#newMsg").val();
        if(sendersid != ""){
            $.post("<?= site_url('admin/member_chat_admin/sendMessage'); ?>",{msg:msg,id: sendersid},function(a){
                    $("#newMsg").val("");
                    //attachments = "";
                }
            )
