        ...
        <div class="row">
            <div class="col-lg-12">
                <ul class="breadcrumb">
                    ...
                    <li><a href="">General In Charges</a></li>
                </ul>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <section class="panel">
                    <header class="panel-heading"> General In Charges</header>
                    <div class="panel-body">
                        ...
                        <table id="myTable" class="table table-hover">
                            ...
                            <?php $i=1; foreach($data as $val){ ?>
                                <tr>
                                    <td><?= $val['groupName'] ?></td>
                                    <td><?= $val['totalMembers'] ?></td>
                                    <td><?php $members = explode(',', $val['membersName']); foreach($members as $v){ echo $v."<br/>"; } ?></td>
                                    <td>
                                    <?php foreach($generals as $val1){ if($val1['groupId'] == $val['groupId']){?>
                                        <?= date('d/M/Y H:i:s', strtotime($val1['lasttime'])) ?><br/>
                                    <?php }} ?>
                                    </td>
                                    <!--<td><?= date('d/M/Y H:i:s', strtotime($val['lasttime'])) ?></td>-->
                                    <td>
                                         <!--href="deleteConff('<?= site_url('admin/deleteGeneral/mempacasGroup/'.$val['userId']."/".$val['groupId']."/same") ?>')"-->
                                        <a data-target="#deleteConf<?= $val['groupId'] ?>" data-toggle="modal" href="#"><i class="fa fa-trash"></i></a></td>
                                    </td>
                                </tr>
                            <?php $i++; } ?>
                            </tbody>
...
<?php foreach($data as $val){ ?>
    <div class='modal fade' id='deleteConf<?= $val['groupId'] ?>' role='dialog' tabindex='-1' aria-labelledby='mySmallModalLabel'>
            ...
            <div class='modal-body'>
            <form action='<?= site_url() ?>/admin/deleteGeneral/mempacasGroup' method="post">
            <select class='chosen-select' name="userId" id="userId">
            <option value=''>Select General In Charge</option>
            <?php foreach($generals as $val1){ if($val['groupId'] == $val1['groupId']){ ?>
            <option value='<?= $val1['userId'] ?>'><?= $val1['name'] ?></option>
            <?php }} ?>
            </select>
            <input type="hidden" value="<?= $val['groupId'] ?>" name="groupId">
            <br/>
            <br/>
...
var $remaining = $('#remaining'),
    ...
    $('#myTable').DataTable({aaSorting: [[2, "asc"]]});
</script>