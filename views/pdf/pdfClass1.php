            ...
            <section class="panel">
                ...
                <table id="myTableMempacas" class="table table-hover table-striped" cellspacing="0" width="100%">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <?php $totalClassDays = $this->data->myquery("SELECT count(distinct(date)) as totalClassDays from `markAttendance` WHERE date LIKE '".$monthsss."'"); ?>
                                                        <?php $members = explode(',', $mempacas['studentId']); ?>
                                                    <?php $i=1; if($monthsss >= date('Y-m%', strtotime($mempacas['createdAt']))){ if($mempacas['studentId'] != ''){ foreach($members as $val1){ if($val1 != ''){ ?>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><?php $attendance = $this->data->myquery("SELECT count(*) as totalDays FROM `markAttendance` WHERE date LIKE '".$monthsss."' AND childId = ".$membersDetail[0]['id']); if(empty($attendance[0]['totalDays'])){ 
                                                                        echo '0%'; 
                                                                    }else{ 
                                                                        echo round((($attendance[0]['totalDays'] / $totalClassDays[0]['totalClassDays']) * 100))."%"; 
                                                                    } ?></td>
                                                            <td><?php $attendance1 = $this->data->myquery("SELECT sum(performance) as totalPerformance, count(*) as totalCount FROM `markAttendance` WHERE date LIKE '".$monthsss."' AND childId = ".$membersDetail[0]['id']); if(empty($attendance1[0]['totalPerformance'])){ 
                                                                        echo '0 / 5'; 
                                                                    }else{ 
                                                                        echo round(($attendance1[0]['totalPerformance'] / $totalClassDays[0]['totalClassDays']), 2)." / 5"; 
                                                                    } ?></td>
                                                            <td><?php $attendance1 = $this->data->myquery("SELECT sum(behaviour) as totalPerformance, count(*) as totalCount FROM `markAttendance` WHERE date LIKE '".$monthsss."' AND childId = ".$membersDetail[0]['id']); if(empty($attendance1[0]['totalPerformance'])){
                                                                        echo '0 / 5'; 
                                                                    }else{ 
                                                                        echo round(($attendance1[0]['totalPerformance'] / $attendance1[0]['totalCount']),2)." / 5"; 
                                                                    } ?></td>
                                                            <td><?php $incidentReport = $this->data->myquery('SELECT * FROM incidentReports WHERE childId = '.$membersDetail[0]['id'].' AND createdAt = "'.$monthsss.'" ORDER BY createdAt desc'); if(empty($incidentReport)){ 
                                                                        echo 'No Recent Incident'; 
                                                                    }else{ 
                                                                        echo $incidentReport[0]['anyRecentIncident']; 
                                                                    } ?></td>
                                                            <td><?php $attendanceRemark = $this->data->myquery('SELECT * FROM `markAttendance` WHERE childId = '.$membersDetail[0]['id'].' AND createdAt = "'.$monthsss.'" ORDER BY createdAt desc'); if(empty($attendanceRemark)){ 
                                                                        echo 'Not Remark Yet.'; 
                                                                    }else{ 
                                                        ...
                                                        <?php $i++; }}}}}else{ ?>
                                                        <tr>
                                                            <td colspan="10"><center>NO CLASS CREATED THIS MONTH</center></td>
                                                        </tr>
                                                        <?php } ?>
                                                    </tbody>
                                                </table>
