        ...
        <div class="col-md-12">
            <section class="panel">
                <table id="myTable" class="table table-hover">
                            ...
                            <tbody>
                            <?php $i=1; foreach($incidentReports as $val){ ?>
                                <tr>
                                    <td width="15%"><?= date('d-M-Y', strtotime($val['date'])) ?></td>
                                    ...
                                    <td><?php $child = $this->data->fetch('children', array('id' => $val['childId']))[0]; echo $child['fname']." ".$child['lname'] ?></td>
                                    <td><?= ucfirst($child['gender']) ?></td>
                                    <td><?php $parent = $this->data->fetch('member', array('id' => $child['parent_id']))[0]; echo $parent['fname']." ".$parent['lname'] ?></td>
                                    <!--<td></td>-->
                                    <td><?php if(!empty($val['teacherReports'])){ echo ucfirst($val['teacherReports']); }else{ echo 'No Teacher Report'; } ?></td>
                                    <td><?php if(!empty($val['parentComments'])){ echo ucfirst($val['parentComments']); }else{ echo 'No parent Comment'; } ?></td>
                                    <td><?php if(!empty($val['adminComment'])){ echo ucfirst($val['adminComment']); }else{ echo 'No admin comment'; } ?></td>
                                    <td><?= $val['status'] ?></td>
                                </tr>
                            <?php $i++; } ?>
                            </tbody>
