            ...
            <section class="panel">
                ...
                <table border="1" id="myTableUser" class="table table-hover">
                    <thead>
                                                    ...
                                                    <tbody>
                                                        <?php $members = explode(',', $mempacas['membersId']); $monthsss = date('Y-m%', strtotime($monthsss));?>
                                                        <?php //print_r($members); ?>
                                                    <?php $i=1; if(isset($mempacas['membersId'])){ foreach($members as $val1){ ?>
                                                        ...
                                                        // print_r($val1['memberId']);
                                                        // print_r('SELECT * FROM `mempacasMemerResponse` WHERE `createdAt` like ("'.$monthsss.'") and memberId = '.$val1.' ORDER BY createdAt DESC');
                                                        // exit();
                                                        $mempacasGroupMember = $this->data->myquery('SELECT * FROM `mempacasMemerResponse` WHERE `createdAt` like ("'.$monthsss.'") and memberId = '.$val1.' ORDER BY createdAt DESC'); 
                                                        // print_r($mempacasGroupMember);
                                                        ?>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><?php if(empty($mempacasGroupMember[0]['createdAt'])){ echo 'Not Yet Contacted'; }else{ echo date('d-m-Y', strtotime($mempacasGroupMember[0]['createdAt'])); } ?></td>
